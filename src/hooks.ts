...
// --- PDF.js 加载函数 ---
async function loadPdfJs(win: Window) {
  const pdfjsLibPath = `chrome://${addon.data.config.addonRef}/content/pdf.mjs`; // <--- 确认主库文件名和路径
  const workerSrcPath = `chrome://${addon.data.config.addonRef}/content/pdf.worker.mjs`; // <--- 确认 Worker 文件名和路径

  try {
      ztoolkit.log("Hooks: Attempting to load PDF.js library...");
      // 使用 Zotero 的 Sandbox 或直接注入脚本
      // 方法一：使用 Zotero Sandbox (如果可用且适用) - 可能更安全但复杂
      // const sandbox = Zotero.createSandbox(win);
      // await Zotero.evalInSandbox(`import * as pdfjsLib from "${pdfjsLibPath}"; globalThis.pdfjsLib = pdfjsLib;`, sandbox);

      // 方法二：直接注入脚本到窗口 (更常用)
      const script = win.document.createElementNS("http://www.w3.org/1999/xhtml", "script") as HTMLScriptElement;
      script.type = "module"; // 假设 pdf.mjs 是 ES Module
      script.async = false; // 确保按顺序执行
      script.textContent = `
          try {
              import * as pdfjsLib from "${pdfjsLibPath}";
              globalThis.pdfjsLib = pdfjsLib; // 将 pdfjsLib 挂载到全局 window 对象
              if (pdfjsLib.GlobalWorkerOptions) {
                  pdfjsLib.GlobalWorkerOptions.workerSrc = "${workerSrcPath}";
                  console.log("[${addon.data.config.addonName}] PDF.js worker source set to: ${workerSrcPath}");
              } else {
                   console.error("[${addon.data.config.addonName}] pdfjsLib.GlobalWorkerOptions not found after import.");
              }
          } catch (e) {
              console.error("[${addon.data.config.addonName}] Error importing or configuring PDF.js:", e);
          }
      `;
      win.document.documentElement.appendChild(script);
      // 等待脚本加载和执行（可能需要更可靠的方式，如 Promise）
      await Zotero.Promise.delay(200); // 简单延迟等待，可能不够健壮

      // 检查是否加载成功
      if (win.pdfjsLib) {
           ztoolkit.log("Hooks: PDF.js library loaded and configured successfully.");
      } else {
           ztoolkit.log("ERROR: Hooks - PDF.js library failed to load onto window object.");
      }

  } catch (e) {
      ztoolkit.log("ERROR: Hooks - Error loading PDF.js:", e);
  }
}
// --- 结束 PDF.js 加载函数 ---
...
async function onMainWindowLoad(win: Window): Promise<void> {
  ztoolkit.log("Hooks: onMainWindowLoad starting..."); // 添加日志
  ...
  // ... (onMainWindowLoad 的其他代码不变) ...
   ztoolkit.log("Hooks: onMainWindowLoad finished."); // 添加日志
}
// --- 结束修改 onMainWindowLoad ---
